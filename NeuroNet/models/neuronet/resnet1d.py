...
class FrameBackBone(nn.Module):
    ...
    def forward(self, x):
        """
        Hỗ trợ:
        - Single-channel sau make_frame: (B, F, W) hoặc (B, F, 1, W)
        - Multi-channel sau make_frame:  (B, F, C, W)

        Với multi-channel, mỗi channel đi qua cùng FrameBackBone,
        sau đó mean fusion theo chiều channel.
        """
        # Nếu input là (B, F, C, W), xử lý từng channel bằng shared backbone.
        if x.dim() == 4:
            b, f, c, w = x.shape

            # (B, F, C, W) -> (B, C, F, W) -> (B*C, F, W)
            x = x.permute(0, 2, 1, 3).contiguous().view(b * c, f, w)

            latent_seq = []

            for i in range(x.shape[1]):
                sample = torch.unsqueeze(x[:, i, :], dim=1)  # (B*C, 1, W)
                latent = self.model(sample)
                latent_seq.append(latent)

            latent_seq = torch.stack(latent_seq, dim=1)      # (B*C, F, D0)
            latent_seq = self.feature_layer(latent_seq)      # (B*C, F, D)

            d = latent_seq.shape[-1]
            latent_seq = latent_seq.view(b, c, f, d)         # (B, C, F, D)

            # Mean fusion EEG + EOG
            latent_seq = latent_seq.mean(dim=1)              # (B, F, D)

            return latent_seq

        # Nếu input là (B, F, W), xử lý như code gốc.
        latent_seq = []
